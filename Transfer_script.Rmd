...
author: "Janet Bang and Virginia Marchman"
date: "April 15, 2019; updated September 20, 2019"
output:
  html_document: 
   toc: true
   toc_float: true
...
This is the code to generate the analyses in Marchman, Bermudez, Bang & Fernald (2019), Off to a good start: Early Spanish-language processing efficiency supports Spanish- and English-language outcomes at 4 ½ years in sequential bilinguals. 
...
```{r}
# calculate difference scores for sd
# cohen's d
transfer <- transfer %>% 
  mutate(LBQSpan_diff = LBQSpan25m - LBQSpan_4.5yrs, 
         LBQSpanCaregivers_diff = LBQSpan25mCaregivers - LBQSpanCaregivers_4.5yrs)
...
```{r}
# covariates only
m1 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore, 
         data = transfer)

# covariates + lbq
m2 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore, 
         data = transfer)

# covariates + lbq + cdi
m3 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore, 
         data = transfer)

# covariates + lbq + acc
m4 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + acc_zscore, 
         data = transfer)

# covariates + lbq + cdi + acc
m5 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore + acc_zscore, 
         data = transfer)

# covariates + lbq + rt
m6 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + rt_zscore, 
         data = transfer)

# covariates + lbq + cdi + rt
m7 <- lm(Spancomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore + 
           rt_zscore, 
         data = transfer)
...
stargazer(m1, m2, m3, m4, m5, m6, m7, type = "text",
         ...
         covariate.labels=c("SES 4.5y", "Age at 4.5y", "Spanish Language Exposure at 4.5y",
                            "Spanish Vocabulary at 25m", "Spanish Accuracy at 25m",
                            "Spanish RT at 25m"))
...
```{r}
# covariates only
m8 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore, 
         data = transfer)

# covariates + lbq
m9 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore, 
         data = transfer)

# covariates + lbq + cdi
m10 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore, 
         data = transfer)

# covariates + lbq + acc
m11 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + acc_zscore, 
         data = transfer)

# covariates + lbq + cdi + acc
m12 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore + acc_zscore, 
         data = transfer)

# covariates + lbq + rt
m13 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + rt_zscore, 
         data = transfer)

# covariates + lbq + cdi + rt
m14 <- lm(Engcomposite ~ hi_4.5y_zscore + age_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore + 
           rt_zscore, 
         data = transfer)
...
anova(m8, m9, m13)
anova(m8, m9, m14)
stargazer(m8, m9, m10, m11, m12, m13, m14, type = "text",
         star.char = c(".","*","**","***"),
         star.cutoffs = c(.1, .05, .01, .001),
         notes = c(". p<0.1; * p<0.05; ** p<0.01; *** p<0.001"),
         notes.append = F,
         ...
         covariate.labels=c("Age at 4.5y","SES 4.5y","Spanish Language Exposure at 4.5y",
                            "Spanish Vocabulary at 25m", "RT at 25m"))

```
### Partial regression plot: Spanish Composite and Vocabulary
```{r}
# Spanish Composite and Vocab at 25m (from Model 7)

## spanish composite residuals - not including CDI
m_sp_comp <- lm(Spancomposite ~ age_4.5y_zscore + hi_4.5y_zscore + 
           lbq_4.5y_zscore + rt_zscore, data = transfer)
resid_sp_comp <- resid(m_sp_comp)

# cdi residuals - not including spanish composite
m_cdi_resid <- lm(cdi_25m_zscore ~ age_4.5y_zscore + hi_4.5y_zscore + 
           lbq_4.5y_zscore + rt_zscore, data = transfer)
...
ggplot(resid_sp_comp_cdi, aes(resid_cdi, resid_sp_comp)) + 
  ...
  annotate("text", x = 2, y = 26.5, label = "paste(R ^ 2, \" = .167\")", parse = TRUE, size = 6) # change this R2 value for each plot
...
```{r}
# Spanish Composite and RT at 25m (from Model 7)
...
m_rt_resid <- lm(rt_zscore ~ age_4.5y_zscore + hi_4.5y_zscore + 
           lbq_4.5y_zscore + cdi_25m_zscore, data = transfer)
...
ggplot(resid_sp_comp_rt, aes(resid_rt, resid_sp_comp)) + 
  ...
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) + 
  annotate("text", x = 2.5, y = 26.5, label = "paste(R ^ 2, \" = .107\")", parse = TRUE, size = 6) # change this R2 value for each plot
...
```{r}
# English Composite and Vocab at 25m (from Model 14)

## English composite residuals - not including CDI
m_eng_comp <- lm(Engcomposite ~ age_4.5y_zscore + hi_4.5y_zscore + 
           lbq_4.5y_zscore + rt_zscore, data = transfer)
...
ggplot(resid_eng_comp_cdi, aes(resid_cdi, resid_eng_comp)) + 
  ...
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) + 
  annotate("text", x = 2, y = 26.5, label = "paste(R ^ 2, \" = .039\")", parse = TRUE, size = 6) # change this R2 value for each plot
...
```{r}
# English Composite and RT at 25m (from Model 14)
...
ggplot(resid_eng_comp_rt, aes(resid_rt, resid_eng_comp)) + 
  ...
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) + 
  annotate("text", x = 2.5, y = 26.5, label = "paste(R ^ 2, \" = .091\")", parse = TRUE, size = 6) # change this R2 value for each plot
